// this is a copy of two field. needs three fields.
// description 1, description 2, price
 ...
 * @param {Object} props.actions - { renameSection, renameDescription, renameDescriptionTwo, updateAmount, addLine, addTen, deleteOne, deleteTen, deleteSection }
            ...
            {Array.isArray(section.lines) &&
              section.lines.map((line) => (
                <div key={line.id} className={styles.lineStyle}>
                  ...
                  {/* Description Two */}
                  {editingState.descriptionTwo.id === line.id ? (
                    <input
                      type="text"
                      placeholder="description"
                      value={editingState.descriptionTwo.value}
                      onChange={(e) =>
                        editingState.descriptionTwo.setValue(e.target.value)
                      }
                      onBlur={async () => {
                        if (editingState.descriptionTwo.value.trim() !== "") {
                          await safeAction(() =>
                            actions.renameDescriptionTwo(
                              section.id,
                              line.id,
                              editingState.descriptionTwo.value.trim()
                            )
                          );
                        }
                        editingState.descriptionTwo.setId(null);
                      }}
                      onKeyDown={(e) => {
                        if (e.key === "Enter") {
                          e.preventDefault();
                    ...
                    <p
                      onClick={() => {
                        if (!isRefreshing) {
                          editingState.descriptionTwo.setId(line.id);
                          editingState.descriptionTwo.setValue(
                            line.descriptionTwo || ""
                          );
                        }
                      }}
                      style={{
                        ...
                        margin: 0,
                        cursor: isRefreshing ? "default" : "pointer",
                        color: line.descriptionTwo ? "inherit" : "#999",
                        fontStyle: line.descriptionTwo ? "normal" : "italic",
                      }}
                    >
                      {line.descriptionTwo || "Click to add description"}
                    </p>
                  )}

                  {/* Amount */}
                  {editingState.amount.id === line.id ? (
                    <input
                      type="number"
                      value={editingState.amount.value}
                      onChange={(e) =>
                    ...
                    <p
                      onClick={() => {
                        if (!isRefreshing) {
                          editingState.amount.setId(line.id);
                          editingState.amount.setValue(line.amount || "");
                        }
                      }}
                      style={{
                        flex: 1,
